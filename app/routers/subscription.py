...
def user_subscription(
    ...
    elif (USE_CUSTOM_JSON_DEFAULT or USE_CUSTOM_JSON_FOR_NPVTUNNEL) and re.match(r'^Npv[\s_-]?Tunnel/(\d+\.\d+)', user_agent):
        version_str = re.match(r'^Npv[\s_-]?Tunnel/(\d+\.\d+)', user_agent).group(1)
        if LooseVersion(version_str) >= LooseVersion("56.1"):
            conf = generate_subscription(user=user, config_format="v2ray-json", as_base64=False, reverse=False)
            return Response(content=conf, media_type="application/json", headers=response_headers)
        else:
            conf = generate_subscription(user=user, config_format="v2ray", as_base64=True, reverse=False)
